...
初回起動時に画像解析に必要な環境のセットアップが必要な場合があります。

#### 自動セットアップ
通常は自動的に必要な環境がセットアップされます。

#### 手動セットアップが必要な場合
稀に環境セットアップのダイアログが表示される場合があります。以下の手順で解決できます：

1. **アプリケーションディレクトリに移動**
   ```bash
   cd /path/to/creait-code
   ```
...
**注意**: これは画像解析コンポーネントをお使いの環境（Mac/Windows、Intel/ARM等）に最適化するためのもので、アプリの安全性と互換性を確保する正常な動作です。
