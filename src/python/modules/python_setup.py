...
def verify_environment():
    ...
    if missing:
        log_message(f"不足しているパッケージ: {', '.join(missing)}", "WARNING")
        return False
    else:
        log_message("必要なパッケージはすべてインストールされています")

        installed_optional = [pkg for pkg in optional if pkg.lower() in installed]
        missing_optional = [pkg for pkg in optional if pkg.lower() not in installed]
