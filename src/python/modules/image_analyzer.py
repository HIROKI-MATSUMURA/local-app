...
def extract_text(image_data):
    ...
    try:
        # 画像データのデコード処理
        if isinstance(image_data, str):
            logger.info("Base64文字列から画像をデコードします")
            img_data = decode_image(image_data)
            if not img_data:
                logger.warning("画像デコードに失敗しました")
                print("画像デコードに失敗しました", file=sys.stderr, flush=True)
                return {'text': '', 'textBlocks': [], 'error': 'イメージデコードエラー'}
            img = img_data
        elif isinstance(image_data, dict) and 'opencv' in image_data:
            logger.info("OpenCV形式の画像データを使用します")
            img = image_data['opencv']
        elif isinstance(image_data, np.ndarray):
            logger.info("NumPy配列形式の画像データを使用します")
            img = image_data
        else:
            logger.warning(f"非対応の画像データ形式です: {type(image_data)}")
            print(f"非対応の画像データ形式です: {type(image_data)}", file=sys.stderr, flush=True)
            return {'text': '', 'textBlocks': [], 'error': '非対応の画像形式'}

        # 画像の基本情報をログに記録
        if hasattr(img, 'shape'):
            logger.info(f"画像サイズ: {img.shape[1]}x{img.shape[0]}, チャンネル数: {img.shape[2] if len(img.shape) > 2 else 1}")
        else:
            logger.warning("画像サイズ情報が取得できません")

        # まずEasyOCRで試行（利用可能な場合）
        result = None
        if EASYOCR_AVAILABLE:
            try:
                logger.info("EasyOCRによるテキスト抽出を開始します")
                print("EasyOCRによるテキスト抽出を開始します", file=sys.stderr, flush=True)
                result = extract_text_with_easyocr(img)
                
                # エラーの有無を確認
                if result and 'error' in result and result['error']:
                    logger.warning(f"EasyOCRでのテキスト抽出でエラーが発生: {result['error']}")
                    print(f"EasyOCRでのテキスト抽出でエラーが発生: {result['error']}", file=sys.stderr, flush=True)
                    result = None
                else:
                    logger.info("EasyOCRでテキスト抽出完了")
                    print("EasyOCRでテキスト抽出完了", file=sys.stderr, flush=True)
            except Exception as e:
                logger.error(f"EasyOCRでのテキスト抽出に失敗: {e}", exc_info=True)
                print(f"EasyOCRでのテキスト抽出に失敗: {e}", file=sys.stderr, flush=True)
                result = None

        # EasyOCR失敗またはインストールされていない場合はTesseractにフォールバック
        if result is None and TESSERACT_AVAILABLE:
            try:
                logger.info("Tesseractによるテキスト抽出を開始します")
                print("Tesseractによるテキスト抽出を開始します", file=sys.stderr, flush=True)
                result = extract_text_with_tesseract(img)
                logger.info("Tesseractでテキスト抽出完了")
                print("Tesseractでテキスト抽出完了", file=sys.stderr, flush=True)
            except Exception as e:
                logger.error(f"Tesseractでのテキスト抽出に失敗: {e}", exc_info=True)
                print(f"Tesseractでのテキスト抽出に失敗: {e}", file=sys.stderr, flush=True)
                result = {'text': '', 'textBlocks': [], 'error': f'Tesseractエラー: {str(e)}'}
        elif result is None:
            # どちらのOCRも利用できない場合
            logger.warning("OCRエンジンが利用できないため、テキスト抽出をスキップします")
            print("OCRエンジンが利用できないため、テキスト抽出をスキップします", file=sys.stderr, flush=True)
            result = {'text': '', 'textBlocks': [], 'error': 'OCRエンジンが利用できません'}

        # 結果の検証
        if isinstance(result, dict):
            # text, textBlocksが存在することを確認
            if 'text' not in result:
                logger.warning("抽出結果に'text'フィールドがありません、追加します")
                result['text'] = ''
            if 'textBlocks' not in result:
                logger.warning("抽出結果に'textBlocks'フィールドがありません、追加します")
                result['textBlocks'] = []
                
            # 結果のサマリーをログに出力
            text_length = len(result['text']) if isinstance(result['text'], str) else 0
            blocks_count = len(result['textBlocks']) if isinstance(result['textBlocks'], list) else 0
            logger.info(f"テキスト抽出完了: 文字数={text_length}, ブロック数={blocks_count}")
            print(f"テキスト抽出完了: 文字数={text_length}, ブロック数={blocks_count}", file=sys.stderr, flush=True)
            
        return result

    except Exception as e:
        logger.error(f"テキスト抽出エラー: {str(e)}", exc_info=True)
        print(f"テキスト抽出エラー: {str(e)}", file=sys.stderr, flush=True)
        return {'text': '', 'textBlocks': [], 'error': f'抽出処理エラー: {str(e)}'}


def extract_text_with_easyocr(image, min_confidence=0.4):
    """
    EasyOCRを使用して画像からテキストを抽出する
    Windows環境では別プロセスとしてocr_worker.pyを使用して安定性を確保
    ...
    try:
        ...
        logger.error(f"一時ファイル作成エラー: {str(e)}", exc_info=True)
        print(f"一時ファイル作成エラー: {str(e)}", file=sys.stderr, flush=True)
        default_result['error'] = f"一時ファイル作成エラー: {str(e)}"
    ...
    try:
        ...
        try:
            ...
            if not os.path.exists(temp_filename):
                logger.error(f"一時画像ファイルの保存に失敗しました: {temp_filename}")
                print(f"一時画像ファイルの保存に失敗しました: {temp_filename}", file=sys.stderr, flush=True)
                default_result['error'] = "一時画像ファイルの保存に失敗"
                return default_result
                
            logger.info(f"一時画像ファイルを保存しました: {temp_filename} (サイズ: {os.path.getsize(temp_filename)} bytes)")
            
        except Exception as img_err:
            logger.error(f"画像前処理中にエラーが発生: {str(img_err)}", exc_info=True)
            print(f"画像前処理中にエラーが発生: {str(img_err)}", file=sys.stderr, flush=True)
            default_result['error'] = f"画像前処理エラー: {str(img_err)}"
            return default_result
        ...
        try:
            ...
            script_dir = os.path.dirname(os.path.dirname(current_dir))  # src/python
            worker_path = os.path.join(script_dir, 'ocr_worker.py')
            
            # ファイルが存在しない場合の代替パスを確認
            if not os.path.exists(worker_path):
                # モジュールと同じディレクトリにある可能性
                worker_path = os.path.join(current_dir, '..', 'ocr_worker.py')
                worker_path = os.path.abspath(worker_path)
            
            if not os.path.exists(worker_path):
                logger.error(f"OCRワーカースクリプトが見つかりません: {worker_path}")
                print(f"OCRワーカースクリプトが見つかりません: {worker_path}", file=sys.stderr, flush=True)
                default_result['error'] = "OCRワーカースクリプトが見つかりません"
                return default_result
                
            logger.info(f"OCRワーカースクリプトのパス: {worker_path}")
        except Exception as path_err:
            logger.error(f"ワーカーパス取得エラー: {str(path_err)}", exc_info=True)
            print(f"ワーカーパス取得エラー: {str(path_err)}", file=sys.stderr, flush=True)
            default_result['error'] = f"ワーカーパス取得エラー: {str(path_err)}"
            return default_result
        ...
        # オプションをJSON形式に変換
        options_json = json.dumps(ocr_options)
        
        # サブプロセスとしてOCRワーカーを実行
        try:
            logger.info(f"OCRワーカープロセスを開始します: {worker_path}")
            print(f"OCRワーカープロセスを開始します: {worker_path}", file=sys.stderr, flush=True)
            
            # Pythonの実行パスを取得（sys.executableでPythonインタープリターのパスを取得）
            python_exe = sys.executable
            
            # コマンドを実行
            cmd = [python_exe, worker_path, temp_filename, '--options', options_json]
            ...
            if process.returncode != 0:
                logger.error(f"OCRワーカープロセスが異常終了しました（コード: {process.returncode}）")
                logger.error(f"標準エラー出力: {process.stderr}")
                print(f"OCRワーカープロセスが異常終了しました（コード: {process.returncode}）", file=sys.stderr, flush=True)
                default_result['error'] = f"OCRプロセスエラー（コード: {process.returncode}）: {process.stderr}"
                return default_result
            
            # 出力をJSONとしてパース
            try:
                worker_output = process.stdout.strip()
                if not worker_output:
                    logger.error("OCRワーカーが出力を返しませんでした")
                    print("OCRワーカーが出力を返しませんでした", file=sys.stderr, flush=True)
                    default_result['error'] = "OCRワーカー出力なし"
                    return default_result
                
                result = json.loads(worker_output)
                logger.info(f"OCRワーカー処理完了: ステータス={result.get('status', 'unknown')}")
                
                # エラーチェック
                if 'error' in result and result['error']:
                    logger.error(f"OCRワーカーエラー: {result['error']}")
                    print(f"OCRワーカーエラー: {result['error']}", file=sys.stderr, flush=True)
                    return {
                ...
                if result.get('status') == 'success':
                    # テキストブロックを変換（フォーマットが異なる場合）
                    text_blocks = []
                    for block in result.get('textBlocks', []):
                        if 'bounds' in block:
                            # ocr_worker.pyのフォーマットを変換
                            bounds = block['bounds']
                            text_blocks.append({
                                'text': block['text'],
                    ...
                    return {
                        ...
                        'ocr_duration': result.get('ocr_duration')
                    }
                else:
                    # エラーまたは空の結果
                    logger.warning(f"OCRワーカーステータス: {result.get('status', 'unknown')}")
                    return {
                        'text': result.get('text', ''),
                        'textBlocks': result.get('textBlocks', []),
                        'error': result.get('error', f"OCRステータス: {result.get('status', 'unknown')}")
                    }
                
            except json.JSONDecodeError as json_err:
                logger.error(f"OCRワーカー出力のJSONパースエラー: {str(json_err)}")
                logger.error(f"生の出力: {process.stdout[:200]}...")
                print(f"OCRワーカー出力のJSONパースエラー: {str(json_err)}", file=sys.stderr, flush=True)
                default_result['error'] = f"JSONパースエラー: {str(json_err)}"
                return default_result
                
        except subprocess.TimeoutExpired as timeout_err:
            logger.error(f"OCRワーカープロセスがタイムアウトしました: {str(timeout_err)}")
            print(f"OCRワーカープロセスがタイムアウトしました: {str(timeout_err)}", file=sys.stderr, flush=True)
            default_result['error'] = f"OCRプロセスタイムアウト: {str(timeout_err)}"
            return default_result
            
        except Exception as proc_err:
            logger.error(f"OCRワーカープロセス実行エラー: {str(proc_err)}", exc_info=True)
            print(f"OCRワーカープロセス実行エラー: {str(proc_err)}", file=sys.stderr, flush=True)
            default_result['error'] = f"OCRプロセス実行エラー: {str(proc_err)}"
            return default_result
            
    except Exception as e:
        logger.error(f"EasyOCRでの処理中にエラーが発生: {e}", exc_info=True)
        print(f"EasyOCRでの処理中にエラーが発生: {e}", file=sys.stderr, flush=True)
        default_result['error'] = f"OCR全体エラー: {str(e)}"
        return default_result
    finally:
        # 一時ファイルの削除
        try:
            ...
            logger.warning(f"一時ファイル削除に失敗: {str(clean_err)}")
            pass
...
def extract_colors_from_image(image, num_colors=5, **kwargs):
    ...
    Args:
        image: 入力画像（NumPy配列）
        num_colors: 抽出する色の数（デフォルト: 5）
        
    Returns:
        dict: 抽出された色の情報（{'colors': [ ... ] }形式）
    """
    try:
        # convert to RGB color space if needed
        if len(image.shape) == 3 and image.shape[2] == 4:
            # Alpha channelがある場合はRGBに変換
        ...
        # 必ず辞書形式で返す（JSONシリアライズの一貫性のため）
        return {'colors': result}
        
    except Exception as e:
        logger.error(f"色抽出エラー: {str(e)}", exc_info=True)
        return {'colors': []}
