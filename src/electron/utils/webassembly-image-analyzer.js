 ...
 * Python 版の image_analyzer.py の代替実装です
 */
...
} from './webassembly-bridge-adapter';
...
const decodeImageToMat = async (imageBase64) => {
  try {
    ...
    throw new Error(`画像のデコードに失敗しました: ${error.message}`);
  }
};
 ...
 */
export const extractColors = async (imageData, options = {}) => {
  try {
    // 画像をデコード
 ...
 */
export const extractText = async (imageData, options = {}) => {
  try {
    // Tesseractワーカーを取得
 ...
 */
export const analyzeImageSections = async (imageData) => {
  try {
    // 画像をデコード
    const img = await decodeImageToMat(imageData);
    const height = img.rows;
 ...
 */
export const analyzeLayoutPattern = async (imageData) => {
  try {
    // 画像をデコード
    const img = await decodeImageToMat(imageData);
    const height = img.rows;
 ...
 */
export const detectMainSections = async (imageData) => {
  try {
    // 画像をデコード
    const img = await decodeImageToMat(imageData);
    const height = img.rows;
 ...
 */
export const detectCardElements = async (imageData) => {
  try {
    // 画像をデコード
    const img = await decodeImageToMat(imageData);
    const height = img.rows;
 ...
 */
export const detectFeatureElements = async (imageData) => {
  try {
    // 画像をデコード
    const img = await decodeImageToMat(imageData);
    const height = img.rows;
 ...
 */
export const analyzeAll = async (imageData, options = {}) => {
  try {
...
// 各関数をエクスポート
export {
  extractColors as extractColorsFromImage,
  extractText as extractTextFromImage,
