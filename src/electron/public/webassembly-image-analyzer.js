...
const getTesseractWorker = async () => {
  if (!tesseractWorker) {
    ...
    try {
      // リソース可用性の事前テスト
      console.log('🔍 Tesseractリソースの可用性をテスト中...');

      // workerPathの可用性をテスト
      try {
        const workerResponse = await fetch(options.workerPath);
        if (!workerResponse.ok) {
          throw new Error(`Worker unavailable: ${workerResponse.status}`);
        }
        console.log('✅ Worker URL accessible');
      } catch (fetchError) {
        console.warn('⚠️ Worker URL test failed:', fetchError);
      }
